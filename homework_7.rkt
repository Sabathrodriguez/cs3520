...
(define (interp [a : Exp] [env : Env] [k : Cont]) : Value
  (type-case Exp a
    ...
    [(if0E tst thn els)
     (interp (if (equal? (interp tst env k) (numV 0))
                 thn
                 els)
             env k)]
    [(let/ccE n body)
     ;;(avgE (numE 0) (numE 4) (appE (idE 'k) (list (numE 3)))
     (interp body
             (extend-env (bind n (contV k))
                         env)
             k)]
    [(negE num) (interp num env
                        (doMultK (numV -1) k))]
    [(avgE v-sum next-k) (interp left env
                                                 (doAvgK middle right))]))
    #;[(avgE left middle right) (continue (doDivideK (interp left env 
                                                 (plusSecondK middle env
                                                              (plusSecondK right env k))) k) (numV 3))]
...
(define (num+ [l : Value] [r : Value]) : Value
  (num-op + l r))
(define (num/ [left : Value] [right : Value]) : Value
  (num-op / left right))
(define (num* [l : Value] [r : Value]) : Value
  (num-op * l r))
...
(module+ test
  ...
  (test (interp-expr (parse `{let/cc k {neg {k 3}}}))
        `3)
  (test (interp-expr (parse `{let/cc k {avg 0 4 {k 3}}}))
        `3) 
