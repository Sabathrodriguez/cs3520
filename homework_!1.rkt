...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(truE) (numV 1)]
    [(falsE) (numV 0)]
    [(if0E cond t f) (if (equal? (interp cond env) (numV 0))
                        (interp t env)
                        (interp f env))]
...
(define (typecheck [a : Exp] [tenv : Type-Env])
  (type-case Exp a
    ...
    [(truE) (boolT)]
    [(falsE) (boolT)]
    [(if0E cond t f) (local ([define type-res (typecheck t tenv)])
                       (local ([define cond-type (typecheck cond tenv)])
                         (begin
                           (unify! cond-type (numT) cond)
                           (begin
                             (unify! type-res (typecheck f tenv) t)
                             type-res))))]
...
(define (run-prog [s-exp : S-Exp]) : S-Exp
  (local [(define result (parse s-exp))]
    (type-case Type (typecheck result mt-env)
      [(numT) (number->s-exp (numV-n (interp result mt-env))) ]
      [(arrowT arg res) (begin
      ...
      [(varT is) (type-case Type (resolve (typecheck result mt-env))
                [(numT) (number->s-exp (numV-n (interp result mt-env))) ]
                [(arrowT arg res) `function ]
                [(listofT elements) `list]
                [(boolT) `bool]
                [else (error 'run-prog "not a type")])])))
