...
(module+ test
  ...
  (test (parse `{box {+ 1 2}})
        (boxE (plusE (numE 1) (numE 2))))
  (test (parse `{unbox {box 3}})
        (unboxE (boxE (numE 3))))
  (test (parse `{box? {box 3}})
        (box?E (boxE (numE 3))))
...
(define (compile a-fae env)
  (type-case Exp a-fae
    ...
    [(boxE expr) (code-malloc1 18 (compile expr env))]
    ; get exp from location expr in memory
    [(unboxE expr) (code-malloc1
                    19
                    (compile expr env))]
    [(box?E expr) (code-malloc1
                   20
                   (compile expr env))]))
