...
(module+ test
  (test (parse `{fst 1})
        (fstE (numE 1))) 
  (test (parse `{fst {pair 2 3}})
        (fstE (pairE (numE 2) (numE 3))))
    (test (parse `{snd {pair 2 3}})
        (sndE (pairE (numE 2) (numE 3))))
  (test (parse `2)
        (numE 2))
...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(letE n rhs body) (interp body
                               (extend-env
                                (bind n (delay rhs env (box (none))))
                                env))]
    [(if0E tst thn els)
     (interp (if (equal? (interp tst env) (numV 0))
                 thn
                 els)
             env)]
    [(pairE fst snd) (pairV (delay fst env (box (none))) (delay snd env (box (none))))]
    [(fstE exp)
     (type-case Value (interp exp env)
       [(pairV fst snd) (type-case Thunk fst
                          [(delay body envi done) (force fst)])]
     [else (error 'interp "not a pairV")])]
    [(sndE exp)
    (type-case Value (interp exp env)
       [(pairV fst snd) (type-case Thunk snd
                          [(delay body envi done) (force snd)])]
     [else (error 'interp "not a pairV")])]))

(module+ test
  (test (interp (letE 'x (numE 5) (plusE (idE 'x) (idE 'x))) mt-env)
        (numV 10))
...
(module+ test
  (test (interp-expr (parse `{lambda {x} {1 2}}))
        `function)
  ...
  (test (interp (parse `{pair 1 2}) mt-env)
        (pairV (delay (numE 1) mt-env (box (none))) (delay (numE 2) mt-env (box (none)))))
