...
(define plus `{lambda {a} {lambda {b} {+ a b}}})
(define times `{lambda {a} {lambda {b} {letrec {[f {lambda {n}
                                                     {if0 n
                                                          0
                                                          {+ {f {+ n -1}} a}}}]}
                                         {f b}}}})
...
(module+ test
  ...
  (test (lookup 'y (extend-env
        ...
        (numV 8))
  (test (interp (parse (list->s-exp (list (list->s-exp (list plus `10)) `2))) mt-env)
        (numV 12))
  (test (interp (parse (list->s-exp (list (list->s-exp (list plus `10)) `2))) mt-env)
        (interp (parse (list->s-exp (list `+ `10 `2))) mt-env))
  (test (interp (parse (list->s-exp (list (list->s-exp (list times `2)) `2))) mt-env)
        (numV 4))
  (test (interp (parse (list->s-exp (list (list->s-exp (list times `4)) `5))) mt-env)
        (numV 20))
  (test (interp (parse (list->s-exp (list (list->s-exp (list times `9)) `9))) mt-env)
        (numV 81))
